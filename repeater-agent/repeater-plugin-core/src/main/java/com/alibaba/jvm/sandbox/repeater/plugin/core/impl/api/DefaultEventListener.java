        ...
        try {
            ...
            if (!isTopEvent(event)) {
                if (log.isDebugEnabled()) {
                    log.debug("not top event ,type={},event={},offset={}", invokeType, event.type, eventOffset.get().get());
                }
                return;
            }
            ...
            if (!access(event)) {
                if (log.isDebugEnabled()) {
                    log.debug("event access failed,type={},event={}", invokeType, event.type);
                }
                return;
            }
            ...
            if (processor != null && processor.ignoreEvent((InvokeEvent) event)) {
                if (log.isDebugEnabled()) {
                    log.debug("event is ignore by processor,type={},event={},processor={}", invokeType, event.type, processor);
                }
                return;
            }
            ...
            // uncaught exception
            log.error("[Error-0000]-uncaught exception occurred when dispatch event,type={},event={}", invokeType, event.type, throwable);
