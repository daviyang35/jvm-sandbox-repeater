...
public class DefaultBroadcaster extends AbstractBroadcaster {
    ...
    public RepeaterResult<RecordModel> pullRecord(RepeatMeta meta) {
        ...
        RepeaterResult<String> pr = null;
        try {
            pr = JSONUtil.toBean(resp.getBody(), new TypeReference<RepeaterResult<String>>() {
            });
            if (!pr.isSuccess()) {
            ...
            log.info("unmarshal repeat data error, datasource={}, response={}", meta.getDatasource(), resp);
            return RepeaterResult.builder().success(false).message("repeat data found").build();
        }
