...
public class OpenFeignProcessor extends DefaultInvocationProcessor {
    ...
    public Identity assembleIdentity(BeforeEvent event) {
        ...
        String urlStr = "get url failed";
        String methodName = "get method failed";
        try {
            /**
            ...
            log.error("openfeign plugin assembleIdentity failed", e);
        }
    ...
    public Object assembleResponse(Event event) {
        ...
        try {
            final Object responseStatus = MethodUtils.invokeMethod(response, "status");
            int statusCode = Integer.parseInt(String.valueOf(responseStatus));
...
//                inputStreamField.setAccessible(true);
            ...
            log.error("feign plugin save response failed", e);
        }
        return null;
    }
    ...
    public Object assembleMockResponse(BeforeEvent event, Invocation invocation) {
        try {
            ...
            log.error("feign plugin assembleMockResponse failed, event={}", event.javaClassName + "|" + event.javaMethodName, e);
        }
        return null;
    }
